...
interface OutputBlock {
  type: "text" | "table" | "outline" | "code" | "timestamps" | "scribl";
  title?: string;
  content: string;
  payload?: any;
}
...
/* ────── SCRIBLMOTION BLOCK ────── */
function ScriblMotionEmbed({ payload }: { payload: any }) {
  const { containerRef, loadScene } = useScriblMotion({ autoPlay: true });

  useEffect(() => {
    if (payload) loadScene(payload);
  }, [loadScene, payload]);

  return (
    <div className="w-full h-[400px] bg-[#0a0a0b] border border-zinc-800/50 rounded-lg overflow-hidden relative group mt-3">
      <div ref={containerRef} className="absolute inset-0 w-full h-full cursor-crosshair mix-blend-screen" />
      <div className="absolute bottom-3 right-3 px-2 py-1 rounded bg-black/60 backdrop-blur-md border border-white/10 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
        <div className="size-1.5 rounded-full bg-pink-500 animate-pulse" />
        <span className={`${geistMono.className} text-[9px] text-zinc-400 uppercase tracking-widest`}>Interactive Sandbox</span>
      </div>
    </div>
  );
}
...
function getMockResponse(mode: string, query: string): Message {
  ...
  if (mode === "study") {
    return {
      ...base, content: "I've generated an interactive sandbox to demonstrate this concept.", blocks: [
        { type: "text", title: "Concept Overview", content: "Interactive visual mechanics help rapidly build intuition. Here is a live physics-enabled sandbox directly mapped to your query. Try dropping the data packets and moving the green floor platforms around to route them." },
        {
          type: "scribl",
          ...
          payload: {
            "version": "1.0",
            "scene": { "id": "study_demo", "width": 600, "height": 400, "duration": 30000, "background": "#0a0a0b", "coordinateSystem": "cartesian", "frameRate": 60, "physics": true, "gravity": { "x": 0, "y": 9.8 } },
            "layers": [
              {
                "id": "main", "type": "canvas", "zIndex": 1, "camera": { "x": 0, "y": 0, "zoom": 1.0 },
                "entities": [
                  { "id": "gate_label", "type": "shape", "position": { "x": 300, "y": 50 }, "styles": { "shape": "text", "text": "Physics Sandbox", "fontFamily": "Inter, sans-serif", "fontSize": 20, "fill": "#a8b1ff", "align": "center" } },
                  { "id": "btn_drop", "type": "button", "position": { "x": 300, "y": 100 }, "styles": { "label": "Drop Data Packet", "fill": "#ec4899" }, "interactions": [{ "on": "click", "actions": [{ "type": "add_entity", "payload": { "layerId": "main", "entity": { "type": "shape", "position": { "x": 300, "y": 150 }, "draggable": true, "styles": { "shape": "circle", "radius": 15, "fill": "#3b82f6" }, "physics": { "type": "dynamic", "restitution": 0.8 } } } }] }] },
                  { "id": "floor_left", "type": "shape", "position": { "x": 180, "y": 300 }, "rotation": 0.2, "draggable": true, "styles": { "shape": "rect", "width": 150, "height": 10, "fill": "#10b981", "cornerRadius": 4 }, "physics": { "type": "static" } },
                  { "id": "floor_right", "type": "shape", "position": { "x": 420, "y": 300 }, "rotation": -0.2, "draggable": true, "styles": { "shape": "rect", "width": 150, "height": 10, "fill": "#10b981", "cornerRadius": 4 }, "physics": { "type": "static" } }
                ]
              }
            ]
          }
...
export default function CognitiveWorkspace() {
  ...
  const handleSubmit = (text?: string) => {
    ...
    setInput("");
    setIsTyping(true);
    if (["youtube", "research", "creator", "compare", "study"].includes(mode)) setRightPanelOpen(true);
    setTimeout(() => {
        ...
        {isEmpty ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6 relative">
            ...
            <div className="w-full max-w-2xl relative">
              ...
              <div className={`relative rounded-2xl transition-all duration-500 ${inputFocused ? 'p-[2px] shadow-lg shadow-cyan-500/10' : 'p-0'}`}>
                {/* Beam animation background layer with overflow hidden */}
                              ...
                              {!isCollapsed && (
                                <div className="px-5 pb-4 border-t border-zinc-800/15">
                                  {block.type === "table" ? (
                                    ...
                                    <ScriblMotionEmbed payload={block.payload} />
                                  ) : (
        ...
        <aside className="w-[300px] shrink-0 border-l flex flex-col overflow-hidden" style={{ background: 'var(--surface-0)', borderColor: 'var(--surface-border)' }}>
          <div className="h-14 border-b flex items-center justify-between px-5 shrink-0" style={{ borderColor: 'var(--surface-border)' }}>
            <span className={`${geistMono.className} text-[11px] text-zinc-500 uppercase tracking-[0.1em]`}>
              {mode === "youtube" ? "Video Intel" : mode === "research" ? "Research Panel" : mode === "creator" ? "Creator Tools" : mode === "compare" ? "Comparison" : mode === "study" ? "Study Resources" : "Context"}
            </span>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            {mode === "study" && (
              <>
                <div>
                  <div className={`${geistMono.className} text-[10px] text-zinc-600 uppercase tracking-widest mb-3`}>Knowledge Check</div>
                  <div className="bg-zinc-900/40 rounded-xl border border-zinc-800/25 p-4 relative overflow-hidden group mb-4">
                    ...
                    <h4 className="text-[13px] text-zinc-200 font-semibold mb-1 relative mx-auto text-center">Mastery: 42%</h4>
                    <div className="w-full bg-zinc-800 rounded-full h-1.5 mb-2 relative"><div className="bg-cyan-500 h-1.5 rounded-full w-[42%]" /></div>
                    <p className="text-[11px] text-zinc-400 text-center relative">Keep going! You're almost at proficiency.</p>
                  </div>
                </div>
                <div>
                  <div className={`${geistMono.className} text-[10px] text-zinc-600 uppercase tracking-widest mb-3`}>Quick Actions</div>
                  {["Generate 5 Flashcards", "Extract Formulas", "Simulate Interview", "Save to Notebook"].map(a => (
                    <button key={a} className="w-full px-4 py-2 rounded-lg border border-zinc-800/25 hover:border-zinc-700/30 hover:bg-zinc-800/15 text-[12px] text-zinc-400 hover:text-zinc-200 font-medium transition-all mb-1.5 text-left flex items-center gap-2">
                      <Zap size={12} className="text-zinc-500" /> {a}
                    </button>
                  ))}
                </div>
              </>
