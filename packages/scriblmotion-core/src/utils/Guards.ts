...
export const COORDINATE_CLAMP = 100_000;
...
export function measureEntityDepth(entity: ScriblEntityDef, current: number = 1): number {
    if (!entity.children || entity.children.length === 0) return current;
    let max = current;
...
export function countEntities(payload: ScriblScriptPayload): number {
    let count = 0;
    const countEntity = (e: ScriblEntityDef): void => {
        count++;
        if (e.children) e.children.forEach(countEntity);
    };
...
export function countAnimations(payload: ScriblScriptPayload): number {
    let count = 0;
    const countEntity = (e: ScriblEntityDef): void => {
        if (e.animations) count += e.animations.length;
        if (e.children) e.children.forEach(countEntity);
    };
    for (const layer of payload.layers) {
        layer.entities.forEach(countEntity);
...
export function isPayloadWithinLimits(payload: ScriblScriptPayload): boolean {
    return (
        countEntities(payload) <= MAX_ENTITIES &&
