...
/** Internal physics body runtime state. */
interface PhysicsBody {
    entityId: string;
    mass: number;
    inverseMass: number; // 0 for static bodies
    friction: number;
    restitution: number;
    gravityScale: number;
    fixedRotation: boolean;
    /** Previous position for Verlet (not used in Euler mode). */
    prevPosition: Vec2;
    constraints: ConstraintDef[];
    /** Whether this body is currently being dragged (kinematic override). */
    isDragged: boolean;
}

export class PhysicsSystem {
    private readonly _bodies = new Map<string, PhysicsBody>();
    ...
    registerBody(entity: EngineEntity): void {
        ...
        if (!entity.acceleration) entity.acceleration = { x: 0, y: 0 };
    }
     ...
     */
    step(dt: number): void {
        if (dt <= 0 || dt > 0.1) return; // clamp to avoid spiral of death
    ...
    private integrate(dt: number): void {
        for (const [entityId, body] of this._bodies) {
            if (body.inverseMass === 0 || body.isDragged) continue;
    ...
    private solvePin(entity: EngineEntity, constraint: ConstraintDef): void {
    ...
    private solveDistance(entity: EngineEntity, body: PhysicsBody, constraint: ConstraintDef): void {
        if (!constraint.targetId) return;
    ...
    private solveSpring(entity: EngineEntity, body: PhysicsBody, constraint: ConstraintDef): void {
        if (!constraint.targetId && !constraint.anchor) return;
    ...
    private aabbOverlap(a: EngineEntity, b: EngineEntity): boolean {
        const aw = ((a.styles['width'] as number) ?? 50) * a.scale.x;
        const ah = ((a.styles['height'] as number) ?? (a.styles['radius'] as number ?? 25) * 2) * a.scale.y;
        const bw = ((b.styles['width'] as number) ?? 50) * b.scale.x;
        const bh = ((b.styles['height'] as number) ?? (b.styles['radius'] as number ?? 25) * 2) * b.scale.y;
    ...
    private resolveCollision(
        ...
        const aw = ((a.styles['width'] as number) ?? 50) * a.scale.x;
        const ah = ((a.styles['height'] as number) ?? 50) * a.scale.y;
        const bw = ((b.styles['width'] as number) ?? 50) * b.scale.x;
        const bh = ((b.styles['height'] as number) ?? 50) * b.scale.y;
        ...
        if (overlapX < overlapY) {
            ...
            if (bodyA.inverseMass > 0) a.position.x += sign * overlapX * (bodyA.inverseMass / totalInvMass);
            if (bodyB.inverseMass > 0) b.position.x -= sign * overlapX * (bodyB.inverseMass / totalInvMass);

            // Velocity response
            if (a.velocity && bodyA.inverseMass > 0) a.velocity.x *= -restitution;
            if (b.velocity && bodyB.inverseMass > 0) b.velocity.x *= -restitution;
        } else {
            // Separate along Y
            const sign = a.position.y < b.position.y ? -1 : 1;
            if (bodyA.inverseMass > 0) a.position.y += sign * overlapY * (bodyA.inverseMass / totalInvMass);
            if (bodyB.inverseMass > 0) b.position.y -= sign * overlapY * (bodyB.inverseMass / totalInvMass);

            if (a.velocity && bodyA.inverseMass > 0) a.velocity.y *= -restitution;
            if (b.velocity && bodyB.inverseMass > 0) b.velocity.y *= -restitution;
        }
    }
    ...
    private enforceBounds(): void {
        for (const [entityId, body] of this._bodies) {
            ...
            const hw = ((entity.styles['width'] as number) ?? 50) * entity.scale.x / 2;
            const hh = ((entity.styles['height'] as number) ?? 50) * entity.scale.y / 2;

            if (entity.position.x - hw < 0) {
                entity.position.x = hw;
                if (entity.velocity) entity.velocity.x *= -body.restitution;
            }
            ...
            if (entity.position.y - hh < 0) {
                ...
                if (entity.velocity) entity.velocity.y *= -body.restitution;
            }
