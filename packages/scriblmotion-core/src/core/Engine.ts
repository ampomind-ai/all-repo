...
export class Engine {
    ...
    readonly audioSystem: AudioSystem; // Removed 'public readonly' as it's already declared above

    private readonly _renderer: Renderer;
    private readonly _validator: DSLValidator;
    private readonly _parser: DSLParser;
    ...
    private _activePayload: ScriblScriptPayload | null = null;
    ...
    constructor(config: EngineConfig) {
        ...
        (this.eventBus as any).on('audio:play_requested', ({ audioId }: { audioId: string }) => {
        ...
        this.eventBus.on('interaction:triggered', ({ action }) => {
            switch (action.type) {
                ...
                case 'pause':
                    ...
                    break;
                case 'seek': {
                    const time = action.payload?.['time'] as number | undefined;
                    if (time !== undefined) {
                        this.seek(time);
                    }
                    break;
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (this.eventBus as any).on('ui:slider_change', ({ entityId, value }: { entityId: string, value: number }) => {
    ...
    loadScene(json: unknown): void {
        ...
        for (const entity of this.entityManager.entities.values()) {
            if (entity.archetype && this.archetypeRegistry.has(entity.archetype)) {
                this.characterSystem.instantiate(
                    entity.id,
                    entity.archetype,
                    entity.archetypeState,
                );
            }
        }
     ...
     */
    private setEntityProperty(entityId: string, property: string, value: unknown): void {
        const entity = this.entityManager.get(entityId);
        if (!entity) return;
    ...
    private tick(deltaTime: number): void {
        ...
        const mutations = this.animationSystem.evaluate(
            Array.from(this.timeline.tracks.keys()), // Changed to get all track IDs
