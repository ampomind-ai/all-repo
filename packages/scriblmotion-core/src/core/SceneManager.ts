...
export class SceneManager {
    private _activeScene: EngineScene | null = null;
    private readonly _scenes: Map<string, EngineScene> = new Map();

    private readonly _entityManager: EntityManager;
    private readonly _animationSystem: AnimationSystem;
    private readonly _interactionSystem: InteractionSystem;
    ...
    addScene(payload: ScriblScriptPayload): EngineScene {
        ...
        const defaultCamera: ScriblCameraDef = { x: 0, y: 0, zoom: 1, rotation: 0 };

        const layers: EngineLayer[] = payload.layers.map((layerDef) => ({
            id: layerDef.id,
            type: layerDef.type,
    ...
    switchScene(sceneId: string, payload: ScriblScriptPayload): boolean {
        ...
        for (const layerDef of payload.layers) {
            ...
            for (let i = 0; i < layerDef.entities.length; i++) {
                ...
                const entity = this._entityManager.createFromDef(
                    entityDef,
                    layerDef.id,
                    layerDef.zIndex * 1000 + i,
                );
