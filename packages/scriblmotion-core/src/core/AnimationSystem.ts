...
export class AnimationSystem {
    ...
    validateEntityLimit(
        entityId: string,
        tracks: ReadonlyMap<string, AnimationTrack>,
    ): boolean {
        let count = 0;
        for (const track of tracks.values()) {
            if (track.entityId === entityId && track.isActive) count++;
        }
    ...
    applyMutations(mutations: PropertyMutation[], entities: Map<string, EngineEntity>): void {
        for (const mutation of mutations) {
            ...
            if (!entity) continue;
            setNestedProperty(entity as unknown as Record<string, unknown>, mutation.property, mutation.value);
        }
    }
}
